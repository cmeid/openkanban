...
| `openkanban ticket done` | Mark the current ticket done (reads `$OPENKANBAN_TICKET_ID` injected at spawn). A status change, not a shutdown — the session stays alive and `Enter` on the Done card re-attaches to it. |
...
- **Busy guard (the inverse).** The file is *also* pinned at `waiting` through the whole run of an already-approved tool. The activity heartbeat bridges this for a streaming tool, but a **silent** one — e.g. a quiet `go test` in a Bash tool, where Claude shows the command's output region instead of its own spinner — emits no bytes, so the card sat at `waiting` with nothing for the user to do. `activeTurnVisible` lifts that to `working` when the live screen shows an active-turn marker and no prompt. Known footers: `esc to interrupt` (≤2.1.179) and `· x to stop` (≥2.1.181) — both are matched additively so a session on either version is detected correctly. It is ordered strictly **after** the prompt guard, so an on-screen prompt always wins — if any future footer drifts the check fails *safe* (back to showing `waiting` while busy, never hiding a needs-you). The braille spinner glyphs (`⠋…⠏`) from ≤2.1.179 are retained as signals for opencode/gemini/codex.
...
The shared funnel is `internal/ticketsvc`: a small package of free functions (`LinkSession`, `GateAttach`) that both TUI and CLI call for any `agent_session_id` mutation. Storage tolerates duplicates by policy (existing on-disk duplicates aren't auto-migrated), but the runtime gates ensure no NEW duplicate ever launches a session. `openkanban ticket move` (the generic move verb that replaced `ticket in-progress` and `ticket in-review`) routes through `TicketStore.Move`, closing the pre-existing harmonization gap where those CLI verbs bypassed the promote/prune side-effects the UI was firing.
...
When openkanban spawns an agent into a ticket, the child can mark the ticket done itself:

```bash
openkanban ticket done
```

This sets `status: done`, stamps `completed_at`, and sets `agent_status: completed`. The card lands in Done with no manual column-move required. It does **not** end the session: status changes never do, so `Enter` on the Done card re-attaches to the same agent with its scrollback intact.
