...
**On a suspected wedge the watchdog REPORTS — it does NOT `os.Exit`.** It sets the lock-free `s.suspectedWedged` flag (answered on `handleHello` so a newly dialing TUI is told), emits a `daemon_wedged` `SessionEvent` to subscribers (so a connected TUI can offer an operator-driven restart), and dumps goroutines to the log — once per episode. The flag clears when `dispatchSeq` advances again. Rationale: a dispatch wedge does NOT stop the per-session PTY pumps, so the running agents are fine; self-restarting would only destroy live work. With no TUI connected the suspicion is harmless. Recovery is operator-driven (`openkanban daemon restart`), not a unilateral self-kill.
