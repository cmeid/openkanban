...
## Status changes never end sessions

**Exactly one session per ticket, durable.** A status change — in either direction, to any column, from the board or the CLI — must never end a session. Pressing `Enter` on a ticket parked in `done` lands back in the same live process with its scrollback intact. Sessions end on exactly five things: ticket delete (`performTicketCleanup`), project delete (`internal/app`), explicit `x` (`stopAgent`), the exit-guard modal, and the agent's own exit.

`stampTerminalAgentStatus` is what's left of the old `wrapUpSessionForTicket`: a void, teardown-free badge stamp. It runs BEFORE `m.globalStore.Move(...)` in `dropTicket` / `quickMoveTicket` / `quickMoveTicketBackward`, and the pre-Move ordering still matters — its gate ("is the ticket leaving `in_progress`?") reads the **current** status, which `Move`'s call to `SetStatus` mutates in place. It returns no `tea.Cmd`; the callers return `nil` in that slot.

**The stamp is gated on there being no live pane.** `AgentCompleted` is terminal against *both* status sources — the poll handler refuses to downgrade it (`model.go`, `agentStatusResultMsg`) and so does `applyDaemonStatus` (`daemon_subscribe.go`). Stamping it on a session that is still alive and working would freeze the badge at "Completed" until the card returned to `backlog`/`next`/`in_progress`, where `board.SetStatus` demotes it. So a live pane means no stamp; the badge keeps tracking the agent.

Consequences worth knowing, all deliberate:

- The CLI matches. `cmd/ticket_move.go` and `cmd/ticket_done.go` send no `TicketDone`. The 1:1 ticket↔session invariant those call sites used to cite is enforced at the daemon by `handleSpawn`'s per-TicketID dedup, not by killing sessions. `TestNoTicketDoneFromStatusPaths` (`session_preservation_test.go`) pins where the RPC may be called from at all.
- Nothing calls `MarkExpectedCompletion` on a status change any more, so an agent that finishes and exits on its own now delivers `Expected=false`. The `"exited"` arm therefore preserves an existing `AgentCompleted` instead of demoting it — same guard `stopAgent` / `resetSpawnState` use.
- Preserved sessions accumulate: the exit-guard modal lists them on every quit, and a default-mode daemon defers shutdown until they drain. Run the daemon under launchd (`--persistent`) and use `x` to end a session deliberately.
- Re-entry needs no status gate. `spawnAgent`'s pane-state branches return before the `in_progress` check, and `daemon_resync.go` Pass 1 adopts a `PaneView` for every daemon-owned ticket with no status filter (`PreflightListSessions` does the same at startup), so a cold TUI re-adopts a `done` card's session within `daemonResyncInterval`.

`ForceFresh` is the one path where losing a session is intended, and it now works: brief-chooser option `d` clears `AgentSpawnedAt` **and** `ticketsvc.UnlinkSession`s the conversation UUID, and `buildSpawnReq` skips `--resume` when `plan.ForceFresh` is set. Both halves are required — leaving the stale UUID means the back-fill (which only fires on an empty `AgentSessionID`) never claims the new session, so `d` would work once and then silently resume the discarded conversation.

Two seams worth knowing about:

- **`daemonGuardAPI` interface** (`exit_guard.go`) — carries `TicketDone(ctx, ticketID)` so UI tests can substitute a fake without spinning up a real daemon. New daemon RPCs needed from UI code should be added here for the same reason. **The Model field is `m.daemon` (type `daemonAPI`, which embeds `daemonGuardAPI`)** — `m.guardAPI` is a vestige of the pre-PR-#39 name and won't compile. If you copy a call site from a stale branch or older PR diff, sanity-check the receiver before merging.
- **`handleDaemonSessionEvent("exited")` link preservation** (`daemon_subscribe.go`) — `AgentSessionID` and `AgentSpawnedAt` are preserved on **both** expected and unexpected exits. Only the PTY dies; the JSONL transcript outlives the session process, so pulling a ticket back from `done` resumes the same conversation via `--resume`. `AgentStatus` is the meaningful difference: `Expected=true` → `AgentCompleted`; `Expected=false` → `AgentNone` **unless the badge is already `AgentCompleted`**, which is preserved (a finished ticket must not lose its "✓" when its preserved session later exits — see the section above). See commit `c718699` (original unexpected-exit rationale) and PR #158 (extended the same invariant to expected exits).
