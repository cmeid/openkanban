...
- **Keep `Info`-reachable accessors (`Size`/`Running`/`PID`) lock-free** — they read atomic mirrors, never `p.mu`. `Session.Info` calls them while the daemon holds `sessionsMu.RLock`; taking a per-pane lock there re-arms the cascade amplifier (one stuck pane → all RPCs frozen).
